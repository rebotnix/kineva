...
class KinevaDDCSP(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), conv=Conv, ch=()):
        super(KinevaDDCSP, self).__init__()
        self.ch = ch
...
class Model(nn.Module):
    def __init__(self, cfg='kineva.yaml', ch=3, nc=None, anchors=None, is_file=True):  # models, input channels, number of classes
        ...
        if nc and nc != self.yaml['nc']:
            logger.info(f"Overriding models cfg nc={self.yaml['nc']} with nc={nc}")
        ...
        if isinstance(m, (KinevaDDCSP, AnchorFreeDetect, KinIdentity)):
            s = 256  # 2x min stride
    ...
    def fuse(self):  # fuse models Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            elif isinstance(m, (AnchorFreeDetect, KinevaDDCSP)):
                m.fuse()
        self.info()
        return self
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
            ...
            c2 = ch[f] // 2
        elif m in [Detect, IDetect, IAuxDetect, IBin, IKeypoint, AnchorFreeDetect, KinevaDDCSP, KinIdentity]:
            args.append([ch[x] for x in f])
            if not m == KinIdentity:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='kineva.yaml', help='models.yaml')
